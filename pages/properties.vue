	...
	<main class="mb-16 md:mx-20 rounded-lg flex flex-col gap-4 min-h-[40rem]">
		<div class="flex flex-row gap-2 mb-128 h-full" style="grid-template-columns: 25% 75%;">
			...
			<div
				...
				<div class="flex md:hidden w-auto md:w-[65%] h-12 rounded-xl items-center mb-4">
					...
					<NuxtImg src="/svg/search.svg" class="aspect-square bg-white invert h-full rounded-r-xl p-2" />
					<NuxtImg class="h-12 ms-2 w-12 rounded-lg bg-white invert p-2" src="/svg/filter.svg"
						alt="Filter icon" @click="toggleFilters"/>
				</div>

							<div v-if="showFilters" class="md:hidden flex flex-col gap-4 pt-4">
								<div id="box">
									<p class="uppercase font-semibold text-xs">
										Type
									</p>
